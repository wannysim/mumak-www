...
파일 이름은 `src/songs/`에 등록된 `<slug>.json`으로 맞춘다. 또는
`{ "slug": "<slug>", "lyrics": [...] }` 형식으로 slug를 명시할 수 있다.
