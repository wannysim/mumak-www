...
- 발행 단계에 들어가지 못한 ticket의 예약은 72시간 유예 뒤 다음 admission에서 회수한다.
  ...
  복구해도 이전 예약은 남는다.
- version 1 장부에는 예약과 정산을 구분하는 필드가 없다. 정산값은 입력 크기와 영구 파일 크기의
  합이라 예약 상한과 정확히 같아질 수 없으므로, 상한과 같은 값만 미정산 예약으로 읽고 장부의
  마지막 admission 시각을 기준으로 유예를 센다. 나머지 값은 정산으로 남겨 회수하지 않는다.
  읽은 시각이 아니라 기록된 시각을 쓰는 이유는, 예산이 이미 소진된 장부에서는 admission이
  storage_limit으로 실패해 장부를 쓰지 못하고 기준 시각도 저장되지 않기 때문이다.
- 이 해석은 예약 상한에서 중단된 version 1 항목 중 영구 객체를 일부 남긴 경우를 실제 사용량보다
  적게 볼 수 있다. 장부는 결제 사용량이 아니므로, 회수 뒤 실제 사용량은 R2 대시보드에서 확인한다.
