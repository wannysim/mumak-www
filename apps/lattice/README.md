...
- MediaPipe HandLandmarker(웹캠)로 손 추적: 칩을 핀치해 놓으면 존 생성, 몸통 핀치로 이동,
  변/꼭짓점(가장자리 16px) 핀치로 해당 방향 리사이즈
- 비디오 레이어와 웹캠 PIP(CAM_00)도 같은 방식으로 이동/리사이즈 가능 (닫기는 존만)
- 마우스도 손과 동일: 칩을 드래그해 놓은 자리에 존 생성(클릭은 화면 중앙 생성), 박스 드래그로 이동/리사이즈
