...
> 이관 기록(2026-09-17 develop 편입): 이 문서는 `docs/blog-homeserver-plan` 브랜치에 있던 루트
...
- 이미지 인프라는 **조건부**였다. 이 판단은 폐기됐다 — 현행 기준은 [`apps/admin/docs/architecture.md`](../../admin/docs/architecture.md)다.
...
## Phase 4 — 이미지 자산·업로드 (**폐기 · 2026-09-10**)

당시 결론은 "런타임 업로드 인프라를 구축하지 않는다"였고, 근거 문서는 같은 브랜치의
`image-upload-design.md`(MinIO + imgproxy 설계를 거쳐 '배포 안 함'으로 종결)였다. 그 문서는
develop에 옮기지 않았다.

실제로는 릴리즈 `1.17.0`에서 admin을 Vercel + Cloudflare R2로 전환해 업로드 경로를 배포했다.
브라우저 → private R2 staging 직접 업로드 → Vercel 검증·변환 → public R2 발행이고, 운영자는
`admin.wannysim.com`에서 로그인해 `img.wannysim.com` 주소가 담긴 MDX snippet을 받는다.
