...
export const mdxComponents: MDXComponents = {
  ...
  h1: ({ children }) => (
    <h1 id={normalizeHeadingToAnchor(extractText(children))} className="scroll-mt-20 text-3xl font-bold mt-8 mb-4">
      {children}
    </h1>
  ),
  h2: ({ children }) => (
    <h2 id={normalizeHeadingToAnchor(extractText(children))} className="scroll-mt-20 text-2xl font-semibold mt-6 mb-3">
      {children}
    </h2>
  ),
  h3: ({ children }) => (
    <h3 id={normalizeHeadingToAnchor(extractText(children))} className="scroll-mt-20 text-xl font-semibold mt-4 mb-2">
      {children}
    </h3>
  ),
  h4: ({ children }) => (
    <h4 id={normalizeHeadingToAnchor(extractText(children))} className="scroll-mt-20 text-lg font-semibold mt-4 mb-2">
      {children}
    </h4>
  ),
  h5: ({ children }) => (
    <h5 id={normalizeHeadingToAnchor(extractText(children))} className="scroll-mt-20 text-base font-semibold mt-4 mb-2">
      {children}
    </h5>
  ),
  h6: ({ children }) => (
    <h6 id={normalizeHeadingToAnchor(extractText(children))} className="scroll-mt-20 text-sm font-semibold mt-4 mb-2">
      {children}
    </h6>
  ),
