...
`test:ci`는 Vitest와 Python 3 표준 라이브러리 기반 exporter/publisher 테스트를 함께 실행합니다. `.github/app-config/apps.yml`에 `quant`를 등록해 앱별 CI와 E2E 검사 대상에 포함합니다. 로컬 E2E는 환경 변수 없는 미연결 화면을 검증하며, 실제 Supabase 연결·메일 수신·로그인은 별도 운영 검증입니다.
...
1. [001_dashboard.sql](./supabase/migrations/001_dashboard.sql)과 [002_paper_publisher.sql](./supabase/migrations/002_paper_publisher.sql)을 순서대로 적용합니다. 모의 장부의 읽기 전용 발행은 [scripts/README.md](./scripts/README.md)를 참고하세요.
