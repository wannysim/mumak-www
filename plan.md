...
- 이미지 인프라는 **조건부**다. 착수 조건과 최종 아키텍처의 유일한 기준은 [image-upload-design.md](image-upload-design.md)다.
...
## Phase 4 — 이미지 자산·업로드 (조건부)

이미지 자산과 업로드 아키텍처의 유일한 규범 문서는
[image-upload-design.md](image-upload-design.md)다. 이 계획은 상태와 연결점만 기록한다.

- 현재 런타임 업로드 인프라는 구축하지 않는다.
- 즉시 작업은 `apps/blog/mdx-components.tsx`의 고정 `800×400` 제거와 MDX 웹/RSS 계약 검증이다.
- 런타임 업로드 구축은 설계문서 §5의 착수 게이트 중 하나가 충족될 때만 시작한다.
...
이 절은 포인터일 뿐이며 규범이 아니다. 실행 순서와 인수 조건은
[image-upload-design.md](image-upload-design.md)의 §14–15에만 둔다.
