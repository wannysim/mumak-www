...
- `apps/admin`, `apps/blog`, `apps/mumak-next`, `apps/mumak-react`, `apps/mumak-native`가 Playwright E2E 대상.
...
- 개발 URL은 `http://{name}.localhost:1355` 형태다. 예: `http://admin.mumak.localhost:1355`, `http://next.mumak.localhost:1355`, `http://react.mumak.localhost:1355`, `http://blog.mumak.localhost:1355`, `http://native.mumak.localhost:1355`.
...
- 현재 E2E 포트는 `mumak-next: 3000`, `mumak-react: 3001`, `blog: 3002`, `mumak-native: 3003`, `lattice: 3004`, `karaoke: 3005`, `admin: 3006`이다. 새 앱을 추가하면 중복되지 않게 배정한다.
